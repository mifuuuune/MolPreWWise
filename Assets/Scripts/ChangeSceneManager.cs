...
public class ChangeSceneManager : NetworkBehaviour
{
    ...
    public void CmdChange()
    {
        ...
        Prototype.NetworkLobby.CostomLobbyManager2.s_Singleton.ServerChangeScene("Level 1");
    }
    ...
    public void RpcSaveSpawn()
    {
        ...
        if (GameObject.FindWithTag("Loin").GetComponent<NetworkIdentity>().hasAuthority)
        {
        ...
        else if (GameObject.FindWithTag("Bean").GetComponent<NetworkIdentity>().hasAuthority)
        {
            //Debug.Log("bean -> ho autorità");
            GameObject.FindWithTag("Bean").transform.position = new Vector3(-9.5f, 5.0f, -33.5f);
        }
        else if (GameObject.FindWithTag("Sage").GetComponent<NetworkIdentity>().hasAuthority)
        {
