                ...
                || Physics.OverlapSphereNonAlloc(transform.position, HensParametersManager.HenFOV, NearbyPlayers, PlayersLayer) > 0)
            {
                ...
                if (Ignore >= 0.9f || (Ignore >= 0.75f && NearestPlayer.GetComponent<BasicController>().IsTheMole()))
                {
                    if ((Personality == HenPersonalities.PROTECTIVE && Physics.OverlapSphereNonAlloc(transform.position, HensParametersManager.HenProtectiveFOV, NearbyChicks, ChicksLayer) > 0))
                    {
                        ...
                        {
                            if (Vector3.Distance(NearestPlayer.transform.position, transform.position) > HensParametersManager.HenAttackFOV) GetComponent<GoToBehaviour>().ExecuteBehaviour(NearbyPlayers);
                            else GetComponent<EngageBehaviour>().ExecuteBehaviour(NearbyPlayers);
                        }
                    }
                    else if ((Personality == HenPersonalities.UNLIKABLE && Physics.OverlapSphereNonAlloc(transform.position, HensParametersManager.HenProtectiveFOV, NearbyChicks, ChicksLayer) > 0))
                    {
                        ...
                        if (RandomTarget < 0.5f) GetComponent<EngageBehaviour>().ExecuteBehaviour(NearbyPlayers);
                        else
                        {
                            NearestChick = FindNearest(NearbyChicks);
                            if (Vector3.Distance(NearestChick.transform.position, transform.position) > HensParametersManager.HenAttackFOV) GetComponent<GoToBehaviour>().ExecuteBehaviour(NearbyChicks);
                            else GetComponent<EngageBehaviour>().ExecuteBehaviour(NearbyChicks);
                        }
                    }
                    else
                    {
                        if ((Personality == HenPersonalities.COWARD) && Physics.OverlapSphereNonAlloc(transform.position, HensParametersManager.HenCowardFOV, NearbyHens, HensLayer) == 0)
